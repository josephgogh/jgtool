...
public abstract class AbstractFieldBuilder {
    ...
    public String build(final Field field, final Column column) {
        ...
        if (StrUtil.isNotEmpty(column.defaultValue())) {
            //默认值不为空
    ...
    public String buildModify(Field field, Column column, FieldStructure fieldStructure) {
        if (this.fieldType.equals(fieldStructure.getType().toLowerCase())) {
            //类型相等，不修改
            return null;
        }
        StringBuilder result = new StringBuilder();
        ...
        if (column.isNull() && !fieldStructure.isNull()) {
            //判断字段类型可以为空的时候，但是原字段不为空，则设置为空
            result.append(" null ");
        } else if (!column.isNull() && fieldStructure.isNull()) {
            //判断字段类型不为空的时候，且原字段为空，则设置不为空
            result.append(" not null ");
        }
        if (column.isKey() && !fieldStructure.isKey()) {
            //判断是否为主键,原自动不能为主键
            result.append(" primary key ");
        }
        if (StrUtil.isNotEmpty(column.defaultValue())) {
